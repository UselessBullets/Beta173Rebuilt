...
public abstract class Entity
{
    ...
    public boolean isFree(final double xa, final double ya, final double za) {
        final AABB cloneMove = this.bb.cloneMove(xa, ya, za);
    ...
    public void moveRelative(float xa, float ya, final float za) {
        float sqrt = Mth.sqrt(xa * xa + ya * ya);
        if (sqrt < 0.01f) {
            return;
        }
        if (sqrt < 1.0f) {
