...
public class Arrow extends Entity
{
    ...
    public Arrow(final Level level, final Mob mob) {
        ...
        this.x -= Mth.cos(this.yRot / 180.0f * 3.1415927f) * 0.16f;
        this.y -= 0.10000000149011612;
        this.z -= Mth.sin(this.yRot / 180.0f * 3.1415927f) * 0.16f;
        this.setPos(this.x, this.y, this.z);
        this.heightOffset = 0.0f;
        this.xd = -Mth.sin(this.yRot / 180.0f * 3.1415927f) * Mth.cos(this.xRot / 180.0f * 3.1415927f);
        this.zd = Mth.cos(this.yRot / 180.0f * 3.1415927f) * Mth.cos(this.xRot / 180.0f * 3.1415927f);
        this.yd = -Mth.sin(this.xRot / 180.0f * 3.1415927f);
    ...
    public void tick() {
        ...
        if (this.xRotO == 0.0f && this.yRotO == 0.0f) {
            ...
            this.yRot = n;
            this.yRotO = n;
            final float n2 = (float)(Math.atan2(this.yd, sqrt) * 180.0 / 3.1415927410125732);
            this.xRot = n2;
            this.xRotO = n2;
        }
        final int tile = this.level.getTile(this.xTile, this.yTile, this.zTile);
        if (tile > 0) {
            Tile.tiles[tile].updateShape(this.level, this.xTile, this.yTile, this.zTile);
            final AABB aabb = Tile.tiles[tile].getAABB(this.level, this.xTile, this.yTile, this.zTile);
        ...
        if (!this.inGround) {
            ++this.flightTime;
            HitResult clip = this.level.clip(Vec3.newTemp(this.x, this.y, this.z), Vec3.newTemp(this.x + this.xd, this.y + this.yd, this.z + this.zd), false, true);
            ...
            for (int i = 0; i < entities.size(); ++i) {
                ...
                if (entity.isPickable()) {
                    if (entity != this.owner || this.flightTime >= 5) {
                        final float n4 = 0.3f;
                        final HitResult clip2 = entity.bb.grow(n4, n4, n4).clip(temp, b);
                        if (clip2 != null) {
            ...
            if (lq != null) {
                clip = new HitResult(lq);
            }
            if (clip != null) {
                if (clip.entity != null) {
                    if (clip.entity.hurt(this.owner, 4)) {
                    ...
                    this.xd = (float)(clip.pos.x - this.x);
                    this.yd = (float)(clip.pos.y - this.y);
                    this.zd = (float)(clip.pos.z - this.z);
                    final float sqrt2 = Mth.sqrt(this.xd * this.xd + this.yd * this.yd + this.zd * this.zd);
                    this.x -= this.xd / sqrt2 * 0.05000000074505806;
                    this.y -= this.yd / sqrt2 * 0.05000000074505806;
                    this.z -= this.zd / sqrt2 * 0.05000000074505806;
                    this.level.playSound(this, "random.drr", 1.0f, 1.2f / (this.random.nextFloat() * 0.2f + 0.9f));
                    this.inGround = true;
                    this.shakeTime = 7;
                }
            ...
            this.y += this.yd;
            this.z += this.zd;
            final float sqrt3 = Mth.sqrt(this.xd * this.xd + this.zd * this.zd);
            this.yRot = (float)(Math.atan2(this.xd, this.zd) * 180.0 / 3.1415927410125732);
            this.xRot = (float)(Math.atan2(this.yd, sqrt3) * 180.0 / 3.1415927410125732);
            ...
            float n5 = 0.99f;
            final float n6 = 0.03f;
            if (this.isInWater()) {
                for (int j = 0; j < 4; ++j) {
                    final float n7 = 0.25f;
                    this.level.addParticle("bubble", this.x - this.xd * n7, this.y - this.yd * n7, this.z - this.zd * n7, this.xd, this.yd, this.zd);
                }
                n5 = 0.8f;
            }
            this.xd *= n5;
        ...
        final int tile2 = this.level.getTile(this.xTile, this.yTile, this.zTile);
        final int data = this.level.getData(this.xTile, this.yTile, this.zTile);
        if (tile2 != this.lastTile || data != this.lastData) {
            this.inGround = false;
    ...
    public void playerTouch(final Player player) {
        ...
        if (this.inGround && this.pickup && this.shakeTime <= 0 && player.inventory.add(new ItemInstance(Item.arrow, 1))) {
