...
public class LevelChunk
{
    ...
    public int getBlocksAndData(final byte[] data, final int x0, final int y0, final int z0, final int x1, final int y1, final int z1, int p) {
        if ((x1 - x0) * (y1 - y0) * (z1 - z0) == this.blocks.length) {
            System.arraycopy(this.blocks, 0, data, p, this.blocks.length);
            p += this.blocks.length;
            System.arraycopy(this.data.data, 0, data, p, this.data.data.length);
            p += this.data.data.length;
            System.arraycopy(this.blockLight.data, 0, data, p, this.blockLight.data.length);
            p += this.blockLight.data.length;
            System.arraycopy(this.skyLight.data, 0, data, p, this.skyLight.data.length);
            p += this.skyLight.data.length;
            return p;
        }
        for (int i = x0; i < x1; ++i) {
            for (int j = z0; j < z1; ++j) {
                ...
                final int n2 = y1 - y0;
                System.arraycopy(this.blocks, n, data, p, n2);
                p += n2;
            }
        }
        for (int k = x0; k < x1; ++k) {
            for (int l = z0; l < z1; ++l) {
                ...
                System.arraycopy(this.data.data, n3, data, p, n4);
                p += n4;
            }
        }
        for (int n5 = x0; n5 < x1; ++n5) {
            for (int n6 = z0; n6 < z1; ++n6) {
                ...
                System.arraycopy(this.blockLight.data, n7, data, p, n8);
                p += n8;
            }
        }
        for (int n9 = x0; n9 < x1; ++n9) {
            for (int n10 = z0; n10 < z1; ++n10) {
                ...
                System.arraycopy(this.skyLight.data, n11, data, p, n12);
                p += n12;
            }
        }
        return p;
