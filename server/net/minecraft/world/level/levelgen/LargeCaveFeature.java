...
public class LargeCaveFeature extends LargeFeature
{
    ...
    protected void addTunnel(final int xOffs, final int zOffs, final byte[] blocks, double xCave, double yCave, double zCave, final float thickness, float yRot, float xRot, int step, int dist, final double yScale) {
        ...
        while (step < dist) {
            ...
            if (!b && step == n6 && thickness > 1.0f) {
                this.addTunnel(xOffs, zOffs, blocks, xCave, yCave, zCave, random.nextFloat() * 0.5f + 0.5f, yRot - 1.5707964f, xRot / 3.0f, step, dist, 1.0);
                this.addTunnel(xOffs, zOffs, blocks, xCave, yCave, zCave, random.nextFloat() * 0.5f + 0.5f, yRot + 1.5707964f, xRot / 3.0f, step, dist, 1.0);
                return;
            }
                        ...
                        for (int n22 = n15; n21 == 0 && n22 < n16; ++n22) {
                            for (int n23 = n19; n21 == 0 && n23 < n20; ++n23) {
                                for (int n24 = n18 + 1; n21 == 0 && n24 >= n17 - 1; --n24) {
                                    ...
                                    if (n24 >= 0) {
                                        if (n24 < 128) {
                                            if (blocks[n25] == Tile.water.id || blocks[n25] == Tile.calmWater.id) {
                                                n21 = 1;
                                            }
                        ...
                        if (n21 == 0) {
                            for (int i = n15; i < n16; ++i) {
                                ...
                                for (int j = n19; j < n20; ++j) {
                                    ...
                                    boolean b3 = false;
                                    if (n26 * n26 + n27 * n27 < 1.0) {
                                        for (int k = n18 - 1; k >= n17; --k) {
                                            final double n29 = (k + 0.5 - yCave) / n8;
                                            if (n29 > -0.7 && n26 * n26 + n29 * n29 + n27 * n27 < 1.0) {
                                                final byte b4 = blocks[n28];
                                                if (b4 == Tile.grass.id) {
                                                    b3 = true;
                                                }
                                                if (b4 == Tile.rock.id || b4 == Tile.dirt.id || b4 == Tile.grass.id) {
                                                    if (k < 10) {
                                                        blocks[n28] = (byte)Tile.lava.id;
                                                    }
                                                    else {
                                                        blocks[n28] = 0;
                                                        if (b3 && blocks[n28 - 1] == Tile.dirt.id) {
                                                            blocks[n28 - 1] = (byte)Tile.grass.id;
                                                        }
                                                    }
                                                }
                                            }
    ...
    protected void addFeature(final Level level, final int x, final int z, final int xOffs, final int zOffs, final byte[] blocks) {
        int nextInt = this.random.nextInt(this.random.nextInt(this.random.nextInt(40) + 1) + 1);
        if (this.random.nextInt(15) != 0) {
            nextInt = 0;
        }
        for (int i = 0; i < nextInt; ++i) {
            final double n = x * 16 + this.random.nextInt(16);
            final double n2 = this.random.nextInt(this.random.nextInt(120) + 8);
            ...
            for (int j = 0; j < n4; ++j) {
                this.addTunnel(xOffs, zOffs, blocks, n, n2, n3, this.random.nextFloat() * 2.0f + this.random.nextFloat(), this.random.nextFloat() * 3.1415927f * 2.0f, (this.random.nextFloat() - 0.5f) * 2.0f / 8.0f, 0, 0, 1.0);
            }
        }
    }
}
