...
public class GhastModel extends Model
{
    ...
    public GhastModel() {
        this.tentacles = new Cube[9];
        final int n = -16;
        (this.body = new Cube(0, 0)).addBox(-8.0f, -8.0f, -8.0f, 16, 16, 16);
        final Cube body = this.body;
        body.y += 24 + n;
        final Random random = new Random(1660L);
        for (int i = 0; i < this.tentacles.length; ++i) {
            this.tentacles[i] = new Cube(0, 0);
            final float x = ((i % 3 - i / 3 % 2 * 0.5f + 0.25f) / 2.0f * 2.0f - 1.0f) * 5.0f;
            final float z = (i / 3 / 2.0f * 2.0f - 1.0f) * 5.0f;
