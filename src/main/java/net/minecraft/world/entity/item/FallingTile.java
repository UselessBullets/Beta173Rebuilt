...
public class FallingTile extends Entity
{
    ...
    public void tick() {
        ...
        if (this.level.getTile(xt, yt, zt) == this.tile) {
        ...
        if (this.onGround) {
            ...
            this.remove();
            if (!this.level.mayPlace(this.tile, xt, yt, zt, true, 1) || SandTile.isFree(this.level, xt, yt - 1, zt) || !this.level.setTile(xt, yt, zt, this.tile)) {
                if (!this.level.isClientSide) {
                    this.spawnAtLocation(this.tile, 1);
                }
            }
        }
        else if (this.time > 20 * 5 && !this.level.isClientSide) {
            this.spawnAtLocation(this.tile, 1);
            this.remove();
        }
    }
