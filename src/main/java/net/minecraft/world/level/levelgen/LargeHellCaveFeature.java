...
public class LargeHellCaveFeature extends LargeFeature
{
    ...
    protected void addTunnel(final int xOffs, final int zOffs, final byte[] blocks, double xCave, double yCave, double zCave, final float thickness, float yRot, float xRot, int step, int dist, final double yScale) {
        ...
        for (; step < dist; step++) {
            ...
            if (!(xCave >= xMid - 16.0 - rad * 2.0) || !(zCave >= zMod - 16.0 - rad * 2.0) || !(xCave <= xMid + 16.0 + rad * 2.0) || !(zCave <= zMod + 16.0 + rad * 2.0)) continue;
            ...
            for (int xx = x0; !detectedWater && xx < x1; ++xx) {
                for (int zz = z0; !detectedWater && zz < z1; ++zz) {
                    for (int yy = y1 + 1; !detectedWater && yy >= y0 - 1; --yy) {
                        ...
                        if (blocks[p] == Tile.lava.id || blocks[p] == Tile.calmLava.id) {
                            detectedWater = true;
                        }
                ...
                for (int zz = z0; zz < z1; ++zz) {
                    ...
                    for (int yy = y1 - 1; yy >= y0; --yy) {
                        ...
                        if (yd > -0.7 && xd * xd + yd * yd + zd * zd < 1.0) {
                            final byte block = blocks[p];
                            if (block == Tile.hellRock.id || block == Tile.dirt.id || block == Tile.grass.id) {
                                blocks[p] = 0;
                            }
                        }
                        p--;
    ...
    protected void addFeature(final Level level, final int x, final int z, final int xOffs, final int zOffs, final byte[] blocks) {
        int caves = this.random.nextInt(this.random.nextInt(this.random.nextInt(10) + 1) + 1);
        if (this.random.nextInt(5) != 0) caves = 0;

        for (int cave = 0; cave < caves; ++cave) {
            final double xCave = x * 16 + this.random.nextInt(16);
            final double yCave = this.random.nextInt(Level.MAX_HEIGHT);
            ...
            for (int i = 0; i < tunnels; ++i) {
                ...
                float thickness = (this.random.nextFloat() * 2.0f + this.random.nextFloat()) * 2.0f;

                this.addTunnel(xOffs, zOffs, blocks, xCave, yCave, zCave, thickness, yRot, xRot, 0, 0, 0.5);
            }
        }
    }
}
