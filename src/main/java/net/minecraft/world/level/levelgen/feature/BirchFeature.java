...
public class BirchFeature extends Feature
{
    ...
    public boolean place(final Level level, final Random random, final int x, final int y, final int z) {
        final int treeHeight = random.nextInt(3) + 5;
        ...
        for (int yy = y - 3 + treeHeight; yy <= y + treeHeight; ++yy) {
            ...
            for (int xx = x - offs; xx <= x + offs; ++xx) {
                ...
                for (int zz = z - offs; zz <= z + offs; ++zz) {
                    ...
                    if (!Tile.solid[level.getTile(xx, yy, zz)]) level.setTileAndDataNoUpdate(xx, yy, zz, Tile.leaves.id, LeafTile.BIRCH_LEAF);
                }
            }
        }
        ...
        for (int hh = 0; hh < treeHeight; ++hh) {
            ...
            if (t == 0 || t == Tile.leaves.id) level.setTileAndDataNoUpdate(x, y + hh, z, Tile.treeTrunk.id, TreeTile.BIRCH_TRUNK);
        }
        return true;
    }
}
